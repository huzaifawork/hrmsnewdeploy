...
// Function to start automatic order progression (called when order is created)
const startAutomaticOrderProgression = (orderId) => {
  if (!orderId) {
    console.error("[Socket] No order ID provided for automatic progression");
    return;
  }

  // Check if already being tracked
  if (activeOrderProgression.has(orderId)) {
    console.log(`[Socket] Order ${orderId} already being tracked`);
    return;
  }

  console.log(`[Socket] Starting automatic progression for order: ${orderId}`);
  ...
  // Start progression with initial status
  console.log(
    `[Socket] Starting progression for order ${orderId} with status: ${statusProgression[0].status}`
  );
  updateOrderStatus(orderId, statusProgression[0].status);
  ...
  statusProgression.slice(1).forEach(({ status, delay }) => {
    const timeout = setTimeout(() => {
      console.log(
        `[Socket] Updating order ${orderId} to status: ${status} after ${delay}ms delay`
      );
      updateOrderStatus(orderId, status);
      ...
      if (status === "Delivered") {
        console.log(
          `[Socket] Order ${orderId} completed, cleaning up progression`
        );
        activeOrderProgression.delete(orderId);
        timeouts.forEach((t) => clearTimeout(t));
        timeouts = [];
  ...
  activeOrderProgression.set(orderId, { active: true, timeouts });
};
...
module.exports = {
  ...
  getIO,
  updateOrderStatus,
  startAutomaticOrderProgression,
};
